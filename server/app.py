...
# JWT compatibility: prefer PyJWT, fall back to python-jose (jose.jwt)
_jwt_mod = None
_jose_mod = None
_use_pyjwt = False
_use_jose = False
...
try:
    ...
    if hasattr(_jwt_mod, 'encode') and hasattr(_jwt_mod, 'decode'):
        _use_pyjwt = True
except Exception:
    _jwt_mod = None
    _use_pyjwt = False
...
if not _use_pyjwt:
    try:
        ...
        if hasattr(_jose_mod, 'encode') and hasattr(_jose_mod, 'decode'):
            _use_jose = True
    except Exception:
        _jose_mod = None
        _use_jose = False
...
def encode_token(payload):
    """Encode JWT payload using available library and return string token."""
    if _use_pyjwt:
        token = _jwt_mod.encode(payload, SECRET_KEY, algorithm='HS256')
        if isinstance(token, bytes):
            token = token.decode('utf-8')
        return token
    if _use_jose:
        return _jose_mod.encode(payload, SECRET_KEY, algorithm='HS256')
    raise RuntimeError('No supported JWT library found. Install PyJWT or python-jose')

def decode_token(token):
    """Decode JWT token using available library. Returns payload or None on failure."""
    if not token:
        return None
    if _use_pyjwt:
        try:
            return _jwt_mod.decode(token, SECRET_KEY, algorithms=['HS256'])
        except Exception:
            return None
    if _use_jose:
        try:
            return _jose_mod.decode(token, SECRET_KEY, algorithms=['HS256'])
        except Exception:
            return None
    return None
...
def register():
    ...
    # Generate JWT token (compat wrapper)
    try:
        token = encode_token({'user_id': user_id, 'email': email})
    except Exception as ex:
        return jsonify({'error': 'Token generation failed', 'details': str(ex)}), 500
...
def login():
    ...
    # Generate JWT token (compat wrapper)
    try:
        token = encode_token({'user_id': user_id, 'email': user_email})
    except Exception as ex:
        return jsonify({'error': 'Token generation failed', 'details': str(ex)}), 500
...
# ...existing code...
# filepath: /home/rebel/Movie-recommender-system/server/app.py
# ...existing code...
from flask import Flask, request, jsonify
from flask_cors import CORS
import pandas as pd
